...
if (!opener.chrome.management) {
  ...
  framer.src = opener
    ...
    : document.domain.includes(".")
    ? "https://" + document.domain
    : "chrome-extension://" + document.domain + "/restricted.html";
...
function removeExtension(extensionId) {
  opener.chrome.management.uninstall(extensionId);
}
...
async function getIconFromExtension(extensionID) {
  ...
  let extensionImages = (
    await new Promise((resolve) => {
      opener.chrome.management.getAll(resolve);
    })
  ).filter((itm) => itm.id == extensionID)[0].icons;
...
function toggleExtension(e, extensionId) {
  if (e.hasAttribute("unchecked")) {
    opener.chrome.management.setEnabled(extensionId, true);
  } else {
    opener.chrome.management.setEnabled(extensionId, false);
  }
}
...
async function getExtensions() {
  const savedata = (
    await new Promise((resolve) => {
      opener.chrome.storage.sync.get("ingotsave", resolve);
    })
  )["ingotsave"];
  console.log(savedata);
  opener.chrome.management.getAll(async function (allExtensions) {
      ...
      if (!allExtensions[anExtension].isApp || true) {
        if (savedata) {
          if (savedata.hasOwnProperty(allExtensions[anExtension].id)) {
            opener.chrome.management.setEnabled(
              allExtensions[anExtension].id,
              savedata[allExtensions[anExtension].id]
            );
            allExtensions[anExtension].enabled =
              savedata[allExtensions[anExtension].id];
          }
        }
        addExtension({
          ...
          logo: allExtensions[anExtension].icons[
            allExtensions[anExtension].icons.length - 1
          ].url,
          managed:
            allExtensions[anExtension].installType == "admin" ? true : false,
  ...
  addSetting({
    ...
    version: "Setting",
    description:
      "Disables iBoss proxy and kills background page when turned off.",
    logo: "",
    id: "proxy",
    managed: false,
...
function reload() {
  ...
  items.forEach(async (item) => {
    if (item.getAttribute("data-id") == "proxy") {
      ...
      opener.chrome.management.get(item.getAttribute("data-id"), (info) => {
        if (info.enabled) {
          item.querySelector(".item-toggle").removeAttribute("unchecked");
        } else {
          item.querySelector(".item-toggle").setAttribute("unchecked", "");
...
function save() {
  ...
  return new Promise((resolve) => {
    opener.chrome.storage.sync.get("ingotsave", (ingotsave) => {
      let extensionStatus = ingotsave.ingotsave || {};
      let items = document.querySelectorAll(".item");
      items.forEach((item) => {
        extensionStatus[item.getAttribute("data-id")] = !item
          .querySelector(".item-toggle")
          .hasAttribute("unchecked");
      });
      opener.chrome.storage.sync.set({ ingotsave: extensionStatus }, resolve);
    });
  });
}
...
async function setIcons() {
  ...
  for (let item in items) {
    try {
      items[item].querySelector(".item-main .item-img-wrapper .item-img").src =
        await getIconFromExtension(items[item].dataset.id);
    } catch {}
  }
}

document.querySelector("iframe").addEventListener("load", () => {
  if (opener || opener.chrome.management) {
